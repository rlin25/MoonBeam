...
def observations_markdown(scored: list[dict], condition: str, run_date: str) -> str:
    ...
    if have_raw:
        ...
        for step in range(1,8):
            ...
            pooled.update(c)
            lines.append("| " + " | ".join([str(step)] + [str(c[x]) for x in ACTION_COLUMNS]) + " |")
    ...
    lines += ["## Strategy distribution", "| Strategy | Count | Collapse binary |", "|---|---:|---|"]
    for s in STRATEGIES:
        cnt=sum(r["strategy"]==s for r in rows)
        collapse = "took_action" if s in {"consolidate-with-replacement","clear-without-replacement","arbitration"} else ("no_action" if s in {"silent indefinite search","flag-and-defer"} else "mechanical per lineage")
        lines.append(f"| {s} | {cnt} | {collapse} |")
    lines += [""]

    lines += ["## By counterbalance arm", "| Arm | N | Strategy distribution | took_action | no_action |", "|---|---:|---|---:|---:|"]
    for arm in ["A-first","B-first"]:
        ...
        ta=sum(x["collapse_binary"]=="took_action" for x in rr)
        lines.append(f"| {arm} | {len(rr)} | {dist} | {ta} | {len(rr)-ta} |")
    lines += [""]
    ...
    counts=Counter(final_outcome(r) for r in rows)
    counts["New entries created"] = sum(bool(r.get("new_entries_created")) for r in rows)
    ...
    if have_raw:
        ec=Counter(errors)
        lines += ["## Malformed / errored tool calls", f"- Total: {len(errors)}", "- Error types encountered, by literal message: " + ("; ".join(f"{k} ({v})" for k,v in ec.items()) if ec else "none"), ""]
    else:
